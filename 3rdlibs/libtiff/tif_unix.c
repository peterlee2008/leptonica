...
/* 
 * for unix, file descriptor is exactly the same as handle. 
 * Here, we just need a union to convert each other
 */
...
_tiffSeekProc(thandle_t fd, uint64 off, int whence)
{
	...
	if((uint64) off_io != off) {
		errno=EINVAL; return (uint64) -1; /* this is really gross */
	}
	fdh.hd = fd;
...
{
	file_client_t fdh; fdh.hd = fd; return(close(fdh.fd));
}

static uint64
_tiffSizeProc(thandle_t fd)
{
	_TIFF_stat_s sb; file_client_t fdh; fdh.hd = fd;
...
{
	(void) fd; (void) munmap(base, (off_t) size);
}
#else /* !HAVE_MMAP */
static int
_tiffMapProc(thandle_t fd, void** pbase, toff_t* psize)
...
{
	TIFF* tif; file_client_t fdh; fdh.fd = fd;
	tif = TIFFClientOpen(name, mode, fdh.hd,
...
TIFFOpenW(const wchar_t* name, const char* mode)
{
	...
	int m, fd; int mbsize; char *mbname = NULL; TIFF* tif;
...
/* for cygwin and mingw */
	...
	tif = TIFFFdOpen(
			  (int)fd, (mbname!=NULL) ? mbname : "<unknown>", mode);
