...
TIFFRGBAImageBegin(TIFFRGBAImage* img, TIFF* tif, int stop, char emsg[1024])
{
    ...
    img->ycbcr = NULL; img->cielab = NULL; img->UaToAa = NULL;
...
{
    char emsg[1024] = ""; TIFFRGBAImage img = {0}; int ok = 0;
    ...
    if (TIFFRGBAImageOK(tif, emsg) && 
        ...
        ok = TIFFRGBAImageGet(
            &img, raster+(rheight-img.height)*rwidth, rwidth, img.height);
    ...
    uint32 rwidth, uint32 rheight, uint32* raster, int stop)
{
    return TIFFReadRGBAImageOriented(
        tif, rwidth, rheight, raster, ORIENTATION_BOTLEFT, stop);
}
...
gtTileContig(TIFFRGBAImage* img, uint32* raster, uint32 w, uint32 h)
{
    ...
    for (row = 0; row < h; row += nrow) {
        ...
        tocol = 0; col = img->col_offset;
        while (tocol < w) {
            if (TIFFReadTile(tif, buf, col, row+img->row_offset,
                0, 0)==(tmsize_t)(-1) && img->stoponerr) {
                    ret = 0; break;
                }
                pos = ((row+img->row_offset) % th) * \
                    TIFFTileRowSize(tif) + \
            ...
            (*put)(img, raster+y*w+tocol, tocol, y, this_tw, nrow, 
                fromskew, this_toskew, buf + pos);
            tocol += this_tw; col += this_tw;
            /*
             * After the leftmost tile, tiles are no longer clipped 
             * on left side.
             */
            fromskew = 0; this_tw = tw; this_toskew = toskew;
        }
...
gtStripSeparate(TIFFRGBAImage* img, uint32* raster, uint32 w, uint32 h)
{
    ...
    for (row = 0; row < h; row += nrow)
    {
        ...
        if (TIFFReadEncodedStrip(
            ...
            )==(tmsize_t)(-1) && img->stoponerr) {
            ret = 0; break;
        }
        if (colorchannels > 1 && 
            TIFFReadEncodedStrip(
                tif, TIFFComputeStrip(tif, offset_row, 1),
                p1, 
                ((row + img->row_offset)%rowsperstrip + nrow) * scanline
            ) == (tmsize_t)(-1) && img->stoponerr) {
            ret = 0; break;
        ...
        if (colorchannels > 1 && 
            TIFFReadEncodedStrip(
                tif, TIFFComputeStrip(tif, offset_row, 2),
                p2, 
                ((row + img->row_offset)%rowsperstrip + nrow) * scanline
            ) == (tmsize_t)(-1) && img->stoponerr) {
            ret = 0; break;
        ...
        if (alpha) {
            if (TIFFReadEncodedStrip(
                    tif, TIFFComputeStrip(tif, offset_row, colorchannels),
                    pa, 
                    ((row+img->row_offset)%rowsperstrip+nrow)*scanline
                ) == (tmsize_t)(-1) && img->stoponerr) {
                ret = 0; break;
        ...
        pos = ((row+img->row_offset) % rowsperstrip) * scanline + \
            ((tmsize_t) img->col_offset * img->samplesperpixel); 
        (*put)(img, raster+y*w, 0, y, w, nrow, fromskew, toskew, 
            p0 + pos, p1 + pos, p2 + pos, (alpha?(pa+pos):NULL)); 
        y += (flip & FLIP_VERTICALLY ? -(int32)nrow : (int32)nrow);
    }
    ...
    if (flip & FLIP_HORIZONTALLY) {
        ...
        for (line = 0; line < h; line++) {
            ...
            while ( left < right ) {
                uint32 temp = *left;
                *left = *right; *right = temp; left++, right--;
            }
        }
    }
...
DECLAREContigPutFunc(putgreytile)
{
    ...
    while (h-- > 0) {
        for (x = w; x-- > 0;) {
            *cp++ = BWmap[*pp][0]; pp += samplesperpixel;
        }
        cp += toskew; pp += fromskew;
    }
}
...
{
    int samplesperpixel = img->samplesperpixel;
    (void) x; (void) y; fromskew *= samplesperpixel;
    while (h-- > 0) {
        UNROLL8(
            w, NOP,
...
{
    int samplesperpixel = img->samplesperpixel;
    (void) y; fromskew *= samplesperpixel;
    while (h-- > 0) {
        uint32 r, g, b, a; uint8* m;
        for (x = w; x-- > 0;) {
            a = pp[3];
            m = img->UaToAa+(a<<8);
            r = m[pp[0]];
...
DECLARESepPutFunc(putRGBUAseparate16bittile)
{
    ...
    while (h-- > 0) {
        ...
        SKEW4(wr, wg, wb, wa, fromskew); cp += toskew;
    }
}
...
DECLARESepPutFunc(putseparate8bitYCbCr11tile)
{
    ...
    (void) a;
    /* 
     * TODO: naming of input vars is still off, change obfuscating 
     * declaration inside define, or resolve obfuscation 
     */
    while (h-- > 0) {
        x = w;
        do {
...
makebwmap(TIFFRGBAImage* img)
{
    ...
    if (img->BWmap == NULL) {
        TIFFErrorExt(img->tif->tif_clientdata, 
            TIFFFileName(img->tif), "No space for B&W mapping table");
        return (0);
    }
...
setupMap(TIFFRGBAImage* img)
{
    ...
    if( img->bitspersample == 16 ) range = (int32) 255;
...
makecmap(TIFFRGBAImage* img)
{
    ...
    if (img->PALmap == NULL) {
        TIFFErrorExt(img->tif->tif_clientdata, 
            TIFFFileName(img->tif), "No space for Palette mapping table");
        return (0);
    }
...
buildMap(TIFFRGBAImage* img)
{
    ...
    if (checkcmap(img) == 16)
        ...
        TIFFWarningExt(img->tif->tif_clientdata, 
            TIFFFileName(img->tif), "Assuming 8-bit colormap");
    /*
...
TIFFReadRGBAStrip(TIFF* tif, uint32 row, uint32 * raster )
{
    ...
    if (TIFFRGBAImageOK(tif, emsg) && 
        ...
        img.row_offset = row; img.col_offset = 0;
...

{
     ...
     */
    img.row_offset = row; img.col_offset = col;
